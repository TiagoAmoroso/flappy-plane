...
Ensure highscore loading doensn't cause an error when the file is empty
...
TubeImage = transformImage(loadImage('Images/cloudCropped.png'),80,600)

menuBackgroundImage = transformImage(loadImage('Images/menuBackgroundImage2.jpg'),screenWidth + 6, screenHeight + 6)
...
class player(object):
    ...
    def hit(self):
        ...
        try:
            if user.highScore > highscore:
                text = font.render('New Highscore!', 1, (0,0,0))
        except:
            print('userHighscore or gameHighscore not defined')
    ...
    def aliveCheck(self):
        ...
        if self.hitbox[0] + self.hitbox[2] > topTube.hitbox[0]:
            if self.hitbox[0] < topTube.hitbox[0] + topTube.hitbox[2]:
                if self.hitbox[1] < topTube.hitbox[1] + topTube.hitbox[3]:
...
class tube(object):
    ...
    def draw(self):
        ...
        window.blit(TubeImage, (self.x,self.y))
...
def positionReset():
    ...
    #backTube = tube(0,-50,60,600)
    bottomTube = tube(screenWidth + 50, random.randint(tubeGap, screenHeight), tubeWidth, 600)
    topTube = tube(bottomTube.x, bottomTube.y - 600 - tubeGap, tubeWidth, bottomTube.y - tubeGap)
...
def redrawGameWindow(update = True, scoreDraw = True):
    #window.fill((0,128,0))
    window.blit(gameBackgroundImage,(-3,-3))
    bottomTube.draw()
    topTube.draw()
...
def backgroundAI():
    ...
    if bottomTube.x < 0 - tubeWidth:
        newYPos = random.randint(tubeGap,screenHeight)
        bottomTube = tube(screenWidth, newYPos, tubeWidth, tubeHeight)

    if topTube.x < 0 - tubeWidth:
        topTube = tube(bottomTube.x,newYPos - tubeGap - 600,tubeWidth, tubeHeight)

    if user.y > bottomTube.y - 65 - random.randint(-10,10):
        if user.canJump:
            user.jump(False)
            user.jumpCount = 30
            user.canJump = False
...
while True:
    ...
    tubeHeight = 600
    tubeGap = 120
    #backTube = tube(0,-50,tubeWidth,600)
    bottomTube = tube(screenWidth + 50, random.randint(tubeGap, screenHeight), tubeWidth, tubeHeight)
    topTube = tube(bottomTube.x, bottomTube.y - 600 - tubeGap, tubeWidth, tubeHeight)
